...
def process_data(input_path, output_path, output_csv_path, val_split_pct):
    ...
    logging.info(disasters)
    disaster_paths = ([input_path + "/" +  d + "/images" for d in disasters])
    image_paths = []
    image_paths.extend([(disaster_path + "/" + pic) for pic in os.listdir(disaster_path) if not pic.startswith('.')] for disaster_path in disaster_paths)
    img_paths = np.concatenate(image_paths)
    #img_paths = img_paths[0:100]
    #img_paths = [s for s in img_paths if 'flood' in s.lower()]
